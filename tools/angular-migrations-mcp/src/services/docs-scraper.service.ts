import { Migration } from '../types/migration.types.js';

export class DocsScraperService {
  private readonly MIGRATIONS_DATA: Migration[] = [
    {
  ...
  async listAvailableMigrations(): Promise<Migration[]> {
    return this.MIGRATIONS_DATA;
  }

  async getMigrationDetails(migrationName: string): Promise<Migration | null> {
    const migration = this.MIGRATIONS_DATA.find(m => m.name === migrationName);
    return migration || null;
  }

  async getMigrationsByCategory(category: string): Promise<Migration[]> {
